...
export async function POST(req: NextRequest) {
  try {
    ...
    const spreadOverMedian: number[] = []
    let botCount = 0
    let sellerCount = 0
    for (const v of result.variants) {
      const prices = v.sellers.map(s=>s.price).filter((n): n is number => Number.isFinite(n))
      if (prices.length >= 2 && typeof v.stats?.median === 'number' && v.stats.median > 0) {
        const min = Math.min(...prices)
        const max = Math.max(...prices)
        const spread = max - min
        spreads.push(spread)
        spreadOverMedian.push(Math.max(0, Math.min(1, spread / v.stats.median)))
      }
    ...
    // Spec: spreadRatio = median(spread/medianPrice)
    const spreadRatio = spreadOverMedian.length ? median(spreadOverMedian) : 0
    // competitionPenalty = 1 - min(1, sellerCount/12)
    const competitionPenalty = 1 - Math.min(1, sellerCount/12)
    // ratingBoost: normalize avg rating (3.5 -> 4.8) to [0,1]
    const ratings = result.variants.map(v=> v.rating?.avg).filter((x): x is number => typeof x==='number')
    const avgRating = ratings.length ? avg(ratings) : 0
    const ratingBoost = clamp01((avgRating - 3.5) / (4.8 - 3.5))
    const attractivenessIndex = Math.round(
      100 * (
        0.40 * (1 - spreadRatio) +
        0.30 * competitionPenalty +
        0.20 * ratingBoost +
        0.10 * (1 - botShare)
      )
    )
