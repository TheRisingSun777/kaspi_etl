...
export default function StoreSelector({ onChange }: { onChange: (id: string)=>void }) {
  ...
  useEffect(()=>{ if (cur) { try { localStorage.setItem('pricebot.storeId', cur) } catch {} onChange(cur); try { await fetch('/api/debug/state', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ storeId: cur }) }) } catch {} } }, [cur])
  return (
